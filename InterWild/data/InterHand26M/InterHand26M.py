...
class InterHand26M(torch.utils.data.Dataset):
    ...
    def load_data(self):
        ...
        # If n_toload is specified and smaller than available samples, randomly sample
        if self.n_toload is not None and self.n_toload < len(aid_list):
            aid_list = random.sample(aid_list, self.n_toload)
            
        # Only load camera params for selected captures
        with open(osp.join(self.annot_path, self.data_split, 'InterHand2.6M_' + self.data_split + '_camera.json')) as f:
            cameras = json.load(f)
            
        # Get unique capture IDs from selected annotations
        needed_captures = set()
        for aid in aid_list:
            ann = db.anns[aid]
            img = db.loadImgs(ann['image_id'])[0]
            needed_captures.add(str(img['capture']))
            
        # Filter camera params to only keep needed captures
        filtered_cameras = {k: v for k, v in cameras.items() if k in needed_captures}
        
        # Load other params similarly, filtering for needed captures
        with open(osp.join(self.annot_path, self.data_split, 'InterHand2.6M_' + self.data_split + '_joint_3d.json')) as f:
            joints = json.load(f)
            filtered_joints = {k: v for k, v in joints.items() if k in needed_captures}
            
        with open(osp.join(self.annot_path, self.data_split, 'InterHand2.6M_' + self.data_split + '_MANO_NeuralAnnot.json')) as f:
            mano_params = json.load(f)
            filtered_mano = {k: v for k, v in mano_params.items() if k in needed_captures}
        ...
        for aid in aid_list:
            ...
            # camera parameters
            t, R = np.array(filtered_cameras[str(capture_id)]['campos'][str(cam)], dtype=np.float32).reshape(3), np.array(filtered_cameras[str(capture_id)]['camrot'][str(cam)], dtype=np.float32).reshape(3,3)
            t = -np.dot(R,t.reshape(3,1)).reshape(3) # -Rt -> t
            focal, princpt = np.array(filtered_cameras[str(capture_id)]['focal'][str(cam)], dtype=np.float32).reshape(2), np.array(filtered_cameras[str(capture_id)]['princpt'][str(cam)], dtype=np.float32).reshape(2)
            ...
            joint_valid = np.array(filtered_joints[str(capture_id)][str(frame_idx)]['joint_valid'], dtype=np.float32).reshape(-1,1)
            ...
            # joint coordinates
            joint_world = np.array(filtered_joints[str(capture_id)][str(frame_idx)]['world_coord'], dtype=np.float32).reshape(-1,3)
            ...
            # mano parameters
            try:
                mano_param = filtered_mano[str(capture_id)][str(frame_idx)].copy()
                if lhand_bbox is None:
