...
class MSCOCO(torch.utils.data.Dataset):
    def __init__(self, transform, data_split, n_toload=None):
        self.transform = transform
        self.data_split = data_split
        self.n_toload = n_toload
    ...
    def load_data(self):
        # Check if a preprocessed dataset exists and if we want all samples
        preprocessed_data_path = osp.join(self.annot_path, f'MSCOCO_{self.data_split}_preprocessed.pth')
        if self.n_toload is None and osp.exists(preprocessed_data_path):
            print(f"Loading preprocessed {self.data_split} dataset...")
            loaded_data = torch.load(preprocessed_data_path)
            return loaded_data['datalist']

        # Load annotations
        if self.data_split == 'train':
            db = COCO(osp.join(self.annot_path, 'coco_wholebody_train_v1.0.json'))
            # Only load MANO params if we're actually going to use them
            if self.n_toload is None or self.n_toload > 0:
        ...
        # Randomly sample if n_toload is specified and smaller than available data
        if self.n_toload is not None and self.n_toload < len(valid_aids):
            valid_aids = np.random.choice(valid_aids, self.n_toload, replace=False)
        ...
        for aid in valid_aids:
            ...
            # Only look up MANO params if we're in training
        ...
        return datalist
    def filter_and_zoom_datalist(self, min_joint_valid=42, min_ratio=0.075, margin_factor=0.2):
        """
        1) Filters self.datalist so that:
        • Each item has at least min_joint_valid valid joints (sum of 'joint_valid').
        • Left/right hand bounding boxes each occupy at least 'min_ratio' of the image width/height.
        2) Zooms/crops the image so that only the rectangle encompassing both hands remains (plus a margin).
        • The margin is margin_factor of the bounding rectangle size.
        • All bounding boxes and joint coordinates are then translated (and optionally scaled) 
            so that (0,0) corresponds to the top-left of the new cropped region.
        """

        def bbox_occupies_min_ratio(bbox, img_shape, ratio):
            """
            Helper that checks if (x2 - x1)/img_width >= ratio and (y2 - y1)/img_height >= ratio.
            """
            if bbox is None:
                return False
            bbox_w = bbox[2] - bbox[0]
            bbox_h = bbox[3] - bbox[1]
            return (bbox_w / img_shape[1] >= ratio) and (bbox_h / img_shape[0] >= ratio)
        
        new_datalist = []
        for item in self.datalist:
            # Basic validity checks
            if item['joint_valid'].sum() < min_joint_valid:
                continue
            lbbox = item['lhand_bbox']
            rbbox = item['rhand_bbox']
            if lbbox is None or rbbox is None:
                continue
            if not bbox_occupies_min_ratio(lbbox, item['img_shape'], min_ratio):
                continue
            if not bbox_occupies_min_ratio(rbbox, item['img_shape'], min_ratio):
                continue
            
            # Combine bounding boxes for both hands
            x1 = min(lbbox[0], rbbox[0])
            y1 = min(lbbox[1], rbbox[1])
            x2 = max(lbbox[2], rbbox[2])
            y2 = max(lbbox[3], rbbox[3])

            w = x2 - x1
            h = y2 - y1

            # Expand by margin_factor (1/5 by default)
            margin_x = margin_factor * w
            margin_y = margin_factor * h

            crop_x1 = max(0, x1 - margin_x)  # clamp to image boundary if needed
            crop_y1 = max(0, y1 - margin_y)
            crop_x2 = min(item['img_shape'][1], x2 + margin_x)
            crop_y2 = min(item['img_shape'][0], y2 + margin_y)

            new_w = crop_x2 - crop_x1
            new_h = crop_y2 - crop_y1

            # Adjust lhand_bbox, rhand_bbox to be relative to the new cropped region
            ...
            lbbox_new = offset_bbox(lbbox)
            rbbox_new = offset_bbox(rbbox)

            # Shift joint coordinates accordingly
            new_joint_img = item['joint_img'].copy()
            if new_joint_img.shape[1] >= 2:  # shift x and y
                new_joint_img[:, 0] -= crop_x1  # shift x
                new_joint_img[:, 1] -= crop_y1  # shift y

            # Create a copy of the item with updated fields, preserving original types
            new_item = dict(item)
            new_item['img_shape'] = tuple(map(int, (new_h, new_w)))  # ensure tuple of ints
            new_item['lhand_bbox'] = lbbox_new  # already numpy float32 from offset_bbox
            new_item['rhand_bbox'] = rbbox_new  # already numpy float32 from offset_bbox
            new_item['joint_img'] = new_joint_img  # numpy array preserved from copy
            new_item['crop_bbox'] = np.array([crop_x1, crop_y1, crop_x2, crop_y2], dtype=np.float32)

            new_datalist.append(new_item)

        # Finally, replace self.datalist with the filtered and updated version
    ...
    def save_preprocessed_data(self):
        """
        Saves the current state of the dataset (datalist and joint_set) to a file.
        """
        preprocessed_data_path = osp.join(self.annot_path, f'MSCOCO_{self.data_split}_preprocessed.pth')
        print(f"Saving preprocessed {self.data_split} dataset to {preprocessed_data_path}...")
        torch.save({
    ...
    def filter_missing_images(self):
        """
        Filters self.datalist, removing items for which the image file is not found.
        """
        new_datalist = []
        for item in self.datalist:
            if osp.exists(item['img_path']):
                new_datalist.append(item)
            else:
                print(f"Warning: Image file not found: {item['img_path']}, skipping.")
        self.datalist = new_datalist
    ...
    def __getitem__(self, idx):
        ...
        # test mode
            ...
            meta_info = {'bb2img_trans': bb2img_trans, 'orig_img_path': img_path}
            return inputs, targets, meta_info
