...
from utils.transforms import restore_bbox, restore_restore_bbox
...
class Model(nn.Module):
    ...
    def forward(self, source_inputs, target_inputs, source_targets, target_targets, meta_info, mode, only_bbox = False, only_hand=False, only_coral_loss=False):
        import math
        spp_levels = [(1, 1)]
        ...
        if not only_hand:
            ...
            source_rhand_bbox = restore_bbox(
                source_targets["rhand_bbox_center"],
                source_targets["rhand_bbox_size"],
                cfg.input_hand_shape[1]/cfg.input_hand_shape[0],
                2.0
            ).detach()
            ...
            source_lhand_bbox = restore_bbox(
                ...
                source_targets["lhand_bbox_size"], 
                cfg.input_hand_shape[1]/cfg.input_hand_shape[0],
                2.0
            ).detach()
        ...
        if not only_hand:
            ...
            source_body_pooled_feats = []
            target_body_pooled_feats = []

            # Get the batch size from the input feature map
            N = source_body_feat.size(0)  # Batch size is the first dimension

            for (ph, pw) in spp_levels:
                source_pooled_feat = F.adaptive_avg_pool2d(source_body_feat, (ph, pw))  # [N, C, ph, pw]
                source_pooled_feat = source_pooled_feat.view(N, -1)  # Flatten spatial dims
                source_body_pooled_feats.append(source_pooled_feat)

                target_pooled_feat = F.adaptive_avg_pool2d(target_body_feat, (ph, pw))
                target_pooled_feat = target_pooled_feat.view(N, -1)
                target_body_pooled_feats.append(target_pooled_feat)

            # CORAL losses
            source_body_feat_spp = torch.cat(source_body_pooled_feats, dim=1)  # Concatenate
            target_body_feat_spp = torch.cat(target_body_pooled_feats, dim=1)
        else:
            target_rhand_bbox = restore_bbox(
                target_targets["rhand_bbox_center"],
                target_targets["rhand_bbox_size"],
                cfg.input_hand_shape[1]/cfg.input_hand_shape[0],
                2.0
            ).detach()

            target_lhand_bbox = restore_bbox(
                target_targets["lhand_bbox_center"],
                target_targets["lhand_bbox_size"],
                cfg.input_hand_shape[1]/cfg.input_hand_shape[0],
                2.0
            ).detach()
        ...
        if not only_bbox:
            ...
            source_hand_pooled_feats = []
            target_hand_pooled_feats = []

            # Get the batch size from the input feature map
            N = source_hand_feat.size(0)  # Batch size is the first dimension

            for (ph, pw) in spp_levels:
                source_pooled_feat = F.adaptive_avg_pool2d(source_hand_feat, (ph, pw))  # [N, C, ph, pw]
                source_pooled_feat = source_pooled_feat.view(N, -1)  # Flatten spatial dims
                source_hand_pooled_feats.append(source_pooled_feat)

                target_pooled_feat = F.adaptive_avg_pool2d(target_hand_feat, (ph, pw))
                target_pooled_feat = target_pooled_feat.view(N, -1)
                target_hand_pooled_feats.append(target_pooled_feat)

            source_hand_feat_spp = torch.cat(source_hand_pooled_feats, dim=1)  # Concatenate
            target_hand_feat_spp = torch.cat(target_hand_pooled_feats, dim=1)

        if mode == 'train':
            # Compute losses
            loss = {}

            if not only_hand and not only_coral_loss:
            ...
            if not only_hand:
                coral_loss = self.CORAL(source_body_feat_spp, target_body_feat_spp)
                loss['coral_body_feat'] = coral_loss * self.coral_loss_weight

            if not only_bbox:
                coral_loss = self.CORAL(source_hand_feat_spp, target_hand_feat_spp)
                loss['coral_hand_feat'] = coral_loss * self.coral_loss_weight

            if not only_bbox and not only_coral_loss:
            ...
            out['source_body_feat_pooled'] = source_body_feat_spp
            out['target_body_feat_pooled'] = target_body_feat_spp
            out['source_hand_feat_pooled'] = source_hand_feat_spp
            out['target_hand_feat_pooled'] = target_hand_feat_spp
            ...
            if not only_hand:
                ...
                out['source_rhand_bbox']  = restore_bbox(source_targets["rhand_bbox_center"], source_targets["rhand_bbox_size"], None, 1.0)
                out['source_lhand_bbox'] = restore_bbox(source_targets["lhand_bbox_center"], source_targets["lhand_bbox_size"], None, 1.0)
                out['source_rhand_bbox_conf'] = 1.0
                out['source_lhand_bbox_conf'] = 1.0
            ...
            if not only_hand:
                ...
                out['target_rhand_bbox']  = restore_bbox(target_targets["rhand_bbox_center"], target_targets["rhand_bbox_size"], None, 1.0)
                out['target_lhand_bbox'] = restore_bbox(target_targets["lhand_bbox_center"], target_targets["lhand_bbox_size"], None, 1.0)
                out['target_rhand_bbox_conf'] = 1.0
                out['target_lhand_bbox_conf'] = 1.0
            ...
            if "target_rhand_bbox" in meta_info:
            ...
            if 'target_rhand_bbox_valid' in meta_info:
