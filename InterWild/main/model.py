...
class Model(nn.Module):
    ...
    def forward(self, inputs, targets, meta_info, mode):
        # body network
        # Interpolate input image to body shape size
        ...
        # Get body features and bounding box predictions
        ...
        (
            ...
            rhand_bbox_conf, 
            lhand_bbox_conf
        ) = self.body_box_net(body_feat)

        # Restore bounding boxes to original image space
        ...
        # Get hand features and transformations
        hand_feat, orig2hand_trans, hand2orig_trans = self.hand_roi_net(
            inputs['img'], 
            rhand_bbox, 
            lhand_bbox
        ) # (2N, ...). right hand + flipped left hand
        
        # Get joint positions and MANO parameters
        joint_img = self.hand_position_net(hand_feat)
        mano_root_pose, mano_hand_pose, mano_shape, root_trans = self.hand_rotation_net(
            hand_feat, 
            joint_img.detach()
        )

        # Split predictions for right and left hands
        ...
        # Flip back left hand x coordinates
        ...
        lhand_pose = torch.cat(
            (lhand_pose[:,:,0:1], -lhand_pose[:,:,1:3]), 
            2
        ).view(lhand_num,-1)

        # Split shape parameters
